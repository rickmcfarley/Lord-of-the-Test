...
function cemen_lorien_grass(data, vi)
	local c_lorgrass1 = minetest.get_content_id("olvar:lorien_grass_1")
	local c_lorgrass2 = minetest.get_content_id("olvar:lorien_grass_2")
	local c_lorgrass3 = minetest.get_content_id("olvar:lorien_grass_3")
	local c_lorgrass4 = minetest.get_content_id("olvar:lorien_grass_4")
...
function cemen_jungletree2(x, y, z, area, data)
	...
	local c_leaves = minetest.get_content_id("olvar:mirkleaf")
	for j = -3, 7 do
		if j == 6 then
			for i = -4, 4 do
			for k = -4, 4 do
...
function add_tree_branch_mirktree(pos)
	...
	for i = math.floor(math.random(2)), -math.floor(math.random(2)), -1 do
		for k = math.floor(math.random(2)), -math.floor(math.random(2)), -1 do
			...
			local n = minetest.env:get_node(p)
			if (n.name=="air") then
				minetest.env:add_node(p, {name="olvar:mirkleaf"})
			end
			local chance = math.abs(i+k)
			if (chance < 1) then
				p = {x=pos.x+i, y=pos.y+1, z=pos.z+k}
