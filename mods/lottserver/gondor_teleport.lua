local teleporter_gondor = {}

teleporter_gondor.reset = function()
  teleporter_gondor.file = minetest.get_worldpath()..'/teleporter_gondor'
  
  local save_contents = ""
  local f = io.open(teleporter_gondor.file, "w")
  f:write(save_contents)
  io.close(f)
  
  teleporter_gondor.file = minetest.get_worldpath()..'/teleporter_build_gondor'
  local save_contents = ""
  local f = io.open(teleporter_gondor.file, "w")
  f:write(save_contents)
  io.close(f)
...
teleporter_gondor.link = function(pos)
  local node_name = ""
  if minetest.env:get_node(pos).name == "lottserver:gondor_teleporter" then
    teleporter_gondor.file = minetest.get_worldpath()..'/teleporter_gondor'
    node_name = "Gondor_Teleporter"
  end

  local save_contents
  print("check if file exists")
  local f = io.open(teleporter_gondor.file, "r")
  if f then
    print("file exists")
...
teleporter_gondor.formspec = function (meta)
  formspec = "size[6,3]" ..
  "label[2,0;Area]"..
...
minetest.register_node("lottserver:gondor_teleporter", {
  description = "Gondorian Teleporter",
  ...
  drop = {
		...
		items = {
			...
			{ items = {'lottserver:gondor_outpost_trophy'} },
		}
   },
  node_box = {
		type = "fixed",
  ...
  on_punch = function(pos, node, puncher, pointed_thing)
    ...
    if (puncher_privs["gondorian"] == true) then
    local meta = minetest.env:get_meta(pos)
    ...
    if newpos.x and newpos.y and newpos.z then
      ...
      for _, obj in pairs(objs) do
        newpos.y = newpos.y -1
        teleporter_gondor.teleport({obj=obj,pos=newpos})
        newpos.y = newpos.y +1.7
        minetest.after(.1, teleporter_gondor.teleport, {obj=obj,pos=newpos})
      end
    end
    end
  ...
  after_place_node = function(pos, placer)
    ...
    meta:set_string("formspec",teleporter_gondor.formspec(meta))
    teleporter_gondor.link(pos)
  end,
...
teleporter_gondor.teleport = function (params)
