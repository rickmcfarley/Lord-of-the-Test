local teleporter_elf = {}

teleporter_elf.reset = function()
  teleporter_elf.file = minetest.get_worldpath()..'/teleporter_elf'
  
  local save_contents = ""
  local f = io.open(teleporter_elf.file, "w")
  f:write(save_contents)
  io.close(f)
  
  teleporter_elf.file = minetest.get_worldpath()..'/teleporter_build_elf'
  local save_contents = ""
  local f = io.open(teleporter_elf.file, "w")
  f:write(save_contents)
  io.close(f)
...
teleporter_elf.link = function(pos)
  local node_name = ""
  if minetest.env:get_node(pos).name == "lottserver:elf_teleporter" then
    teleporter_elf.file = minetest.get_worldpath()..'/teleporter_elf'
    node_name = "Elf_Teleporter"
  end

  local save_contents
  print("check if file exists")
  local f = io.open(teleporter_elf.file, "r")
  if f then
    print("file exists")
...
teleporter_elf.formspec = function (meta)
  formspec = "size[6,3]" ..
  "label[2,0;Area]"..
...
minetest.register_node("lottserver:elf_teleporter", {
  description = "Elven Teleporter",
  tiles = {"lottplants_mallorntree.png"},
  groups = {core_structure=1},
  drop = {
		max_items = 2,
		items = {
			{ items = {'currency:minegeld 30'} },
			{ items = {'lottserver:elf_outpost_trophy'} },
		}
   },
  drawtype = "nodebox",
  node_box = {
  ...
  on_punch = function(pos, node, puncher, pointed_thing)
    ...
    if (puncher_privs["elven"] == true) then
    local meta = minetest.env:get_meta(pos)
    ...
    if newpos.x and newpos.y and newpos.z then
      ...
      for _, obj in pairs(objs) do
        newpos.y = newpos.y -1
        teleporter_elf.teleport({obj=obj,pos=newpos})
        newpos.y = newpos.y +1.7
        minetest.after(.1, teleporter_elf.teleport, {obj=obj,pos=newpos})
      end
    end
    end
  ...
  after_place_node = function(pos, placer)
    ...
    meta:set_string("formspec",teleporter_elf.formspec(meta))
    teleporter_elf.link(pos)
  end,
...
teleporter_elf.teleport = function (params)
