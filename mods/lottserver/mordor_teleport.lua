local teleporter_mordor = {}

teleporter_mordor.reset = function()
  teleporter_mordor.file = minetest.get_worldpath()..'/teleporter_mordor'

  local save_contents = ""
  local f = io.open(teleporter_mordor.file, "w")
  f:write(save_contents)
  io.close(f)

  teleporter_mordor.file = minetest.get_worldpath()..'/teleporter_build_mordor'
  local save_contents = ""
  local f = io.open(teleporter_mordor.file, "w")
  f:write(save_contents)
  io.close(f)
...
teleporter_mordor.link = function(pos)
  local node_name = ""
  if minetest.env:get_node(pos).name == "lottserver:mordor_teleporter" then
    teleporter_mordor.file = minetest.get_worldpath()..'/teleporter_mordor'
    node_name = "mordor_Teleporter"
  end

  local save_contents
  print("check if file exists")
  local f = io.open(teleporter_mordor.file, "r")
  if f then
    print("file exists")
...
teleporter_mordor.formspec = function (meta)
  formspec = "size[6,3]" ..
  "label[2,0;Area]"..
...
minetest.register_node("lottserver:mordor_teleporter", {
  description = "Teleporter of Mordor",
  ...
  drop = {
		...
		items = {
			...
			{ items = {'lottserver:mordor_outpost_trophy'} },
		}
   },
  drawtype = "nodebox",
  node_box = {
  ...
  on_punch = function(pos, node, puncher, pointed_thing)
    ...
    if (puncher_privs["mordor"] == true) then
    local meta = minetest.env:get_meta(pos)
    ...
    if newpos.x and newpos.y and newpos.z then
      ...
      for _, obj in pairs(objs) do
        newpos.y = newpos.y -1
        teleporter_mordor.teleport({obj=obj,pos=newpos})
        newpos.y = newpos.y +1.7
        minetest.after(.1, teleporter_mordor.teleport, {obj=obj,pos=newpos})
      end
    end
    end
  ...
  after_place_node = function(pos, placer)
    ...
    meta:set_string("formspec",teleporter_mordor.formspec(meta))
    teleporter_mordor.link(pos)
  end,
...
teleporter_mordor.teleport = function (params)
