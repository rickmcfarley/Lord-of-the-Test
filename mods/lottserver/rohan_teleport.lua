local teleporter_rohan = {}

teleporter_rohan.reset = function()
  teleporter_rohan.file = minetest.get_worldpath()..'/teleporter_rohan'
  
  local save_contents = ""
  local f = io.open(teleporter_rohan.file, "w")
  f:write(save_contents)
  io.close(f)
  
  teleporter_rohan.file = minetest.get_worldpath()..'/teleporter_build_rohan'
  local save_contents = ""
  local f = io.open(teleporter_rohan.file, "w")
  f:write(save_contents)
  io.close(f)
...
teleporter_rohan.link = function(pos)
  local node_name = ""
  if minetest.env:get_node(pos).name == "lottserver:rohan_teleporter" then
    teleporter_rohan.file = minetest.get_worldpath()..'/teleporter_rohan'
    node_name = "Rohan_Teleporter"
  end

  local save_contents
  print("check if file exists")
  local f = io.open(teleporter_rohan.file, "r")
  if f then
    print("file exists")
...
teleporter_rohan.formspec = function (meta)
  formspec = "size[6,3]" ..
  "label[2,0;Area]"..
...
minetest.register_node("lottserver:rohan_teleporter", {
  ...
  on_punch = function(pos, node, puncher, pointed_thing)
    ...
    if newpos.x and newpos.y and newpos.z then
      ...
      for _, obj in pairs(objs) do
        newpos.y = newpos.y -1
        teleporter_rohan.teleport({obj=obj,pos=newpos})
        newpos.y = newpos.y +1.7
        minetest.after(.1, teleporter_rohan.teleport, {obj=obj,pos=newpos})
      end
    end
    end
  ...
  after_place_node = function(pos, placer)
    ...
    meta:set_string("formspec",teleporter_rohan.formspec(meta))
    teleporter_rohan.link(pos)
  end,
...
teleporter_rohan.teleport = function (params)
