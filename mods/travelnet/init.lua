...
--[[
 ...
 21.06.13 - bugfix: wielding an elevator while digging a door caused the elevator_top to be placed
          ...
          - improved naming of palantiri levels for the elevator
 21.06.13 - elevator palantiris are sorted by height instead of date of creation as is the case with travelnet boxes
          - elevator palantiris are named automaticly
 ...
 20.03.13 - added inventory image provided by VanessaE
          - fixed bug that made it impossible to remove palantiris from the net
          - if the palantiri a player beamed to no longer exists, the palantiri will be removed automaticly
...
-- called whenever a palantiri is added or removed
...
travelnet.update_formspec = function( pos, puncher_name )
   ...
   local palantiri_name    = meta:get_string( "palantiri_name" );
   local palantiri_network = meta:get_string( "palantiri_network" );

   if(  not( owner_name ) 
     or not( palantiri_name ) or palantiri_network == ''
     or not( palantiri_network )) then
...
--                                                  " palantiri_name: "..(palantiri_name or "?")..
--                                               " palantiri_network: "..(palantiri_network or "?")..".");
-- minetest.chat_send_player(puncher_name, "data: "..minetest.serialize(  travelnet.targets ));
      ...
      meta:set_string("formspec", 
                            "size[12,10]"..
                            "field[0.3,7.6;9,0.9;palantiri_name;Name of this palantiri:;"..(palantiri_name or "?").."]"..
                            "field[0.3,8.6;9,0.9;palantiri_network;Assign to Network:;"..(palantiri_network or "?").."]"..
                            "field[0.3,9.6;9,0.9;owner;Owned by:;"..(owner_name or "?").."]"..
                            "button_exit[6.3,8.2;1.7,0.7;palantiri_set;Store]" );

      minetest.chat_send_player(puncher_name, "Error: Update failed! Resetting this box on the travelnet.");
      return;
   end

   -- if the palantiri got lost from the network for some reason (savefile corrupted?) then add it again
   if(  not( travelnet.targets[ owner_name ] )
     or not( travelnet.targets[ owner_name ][ palantiri_network ] )
     or not( travelnet.targets[ owner_name ][ palantiri_network ][ palantiri_name ] )) then
      ...
      -- first palantiri on this network?
      if( not( travelnet.targets[ owner_name ][ palantiri_network ] )) then
         travelnet.targets[       owner_name ][ palantiri_network ] = {};
      end
      ...
      -- add this palantiri
      travelnet.targets[ owner_name ][ palantiri_network ][ palantiri_name ] = {pos=pos, timestamp=zeit };

      minetest.chat_send_player(owner_name, "Palantiri '"..palantiri_name.."' has been reattached to the network '"..palantiri_network.."'.");

   end


   -- add name of palantiri + network + owner + update-button
   local formspec = "size[12,10]"..
                            "label[3.3,0.0;Travelnet-Box:]".."label[6.3,0.0;Punch box to update target list.]"..
                            "label[0.3,0.4;Name of this palantiri:]".."label[6.3,0.4;"..(palantiri_name or "?").."]"..
                            "label[0.3,0.8;Assigned to Network:]" .."label[6.3,0.8;"..(palantiri_network or "?").."]"..
...
--                            "button_exit[5.3,0.3;8,0.8;do_update;Punch box to update destination list. Click on target to travel there.]"..
   ...
   -- collect all palantiri names in a table
   local palantiris = {};
   
   for k,v in pairs( travelnet.targets[ owner_name ][ palantiri_network ] ) do
      table.insert( palantiris, k );
   end
   -- minetest.chat_send_player(puncher_name, "palantiris: "..minetest.serialize( palantiris ));
    
   local ground_level = 1;
   if( is_elevator ) then
      table.sort( palantiris, function(a,b) return travelnet.targets[ owner_name ][ palantiri_network ][ a ].pos.y > 
                                                 travelnet.targets[ owner_name ][ palantiri_network ][ b ].pos.y  end);
      -- find ground level
      local vgl_timestamp = 999999999999;
      for index,k in ipairs( palantiris ) do
         if( travelnet.targets[ owner_name ][ palantiri_network ][ k ].timestamp < vgl_timestamp ) then
            vgl_timestamp = travelnet.targets[ owner_name ][ palantiri_network ][ k ].timestamp;       
            ground_level  = index;
         end
      end
      for index,k in ipairs( palantiris ) do
         if( index == ground_level ) then
            travelnet.targets[ owner_name ][ palantiri_network ][ k ].nr = 'G';
         else
            travelnet.targets[ owner_name ][ palantiri_network ][ k ].nr = tostring( ground_level - index );
         end
      end
            
   else 
      -- sort the table according to the timestamp (=time the palantiri was configured)
      table.sort( palantiris, function(a,b) return travelnet.targets[ owner_name ][ palantiri_network ][ a ].timestamp < 
                                                 travelnet.targets[ owner_name ][ palantiri_network ][ b ].timestamp  end);
   end

   -- if there are only 8 palantiris (plus this one), center them in the formspec
   if( #palantiris < 10 ) then
      x = 4;
   end
   ...
   for index,k in ipairs( palantiris ) do 
      ...
      if( k==palantiri_name ) then
         open_door_cmd = true;
      end

      if( k ~= palantiri_name or open_door_cmd) then 
         i = i+1;
         ...
         elseif( is_elevator ) then
            formspec = formspec .."button_exit["..(x)..","..(y+2.5)..";1,0.5;target;"..tostring( travelnet.targets[ owner_name ][ palantiri_network ][ k ].nr ).."]"..
                                  "label["..(x+0.9)..","..(y+2.35)..";"..tostring( k ).."]";
         else
...
--         if( is_elevator ) then
--            formspec = formspec ..' ('..tostring( travelnet.targets[ owner_name ][ palantiri_network ][ k ].pos.y )..'m)';
--         end
--         formspec = formspec .. ']';
   ...
   meta:set_string( "infotext", "Palantiri '"..tostring( palantiri_name ).."' on network '"..tostring( palantiri_network )..
...
travelnet.add_target = function( palantiri_name, network_name, pos, player_name, meta, owner_name )
...
--      owner_name   = '*'; -- the owner name is not relevant here
      ...
      if( not( palantiri_name ) or palantiri_name == '' ) then
         palantiri_name = 'at '..tostring( pos.y )..'m';
      end
   end

   if( palantiri_name == "" or not(palantiri_name )) then
      minetest.chat_send_player(player_name, "Please provide a name for this palantiri.");
      return;
   end

   if( network_name == "" or not( network_name )) then
      minetest.chat_send_player(player_name, "Please provide the name of the network this palantiri ought to be connected to.");
      return;
   end
   ...
   for k,v in pairs( travelnet.targets[ owner_name ][ network_name ] ) do
      ...
      if( k == palantiri_name ) then
         minetest.chat_send_player(player_name, "Error: A palantiri named '"..palantiri_name.."' already exists on this network. Please choose a different name!");
         return;
      end
   ...
   -- we don't want too many palantiris in the same network because that would get confusing when displaying the targets
   if( anz+1 > travelnet.MAX_STATIONS_PER_NETWORK ) then
      minetest.chat_send_player(player_name, "Error: Network '"..network_name.."' already contains the maximum number (="
              ..(travelnet.MAX_STATIONS_PER_NETWORK)..") of allowed palantiris per network. Please choose a diffrent/new network name.");
      return;
   end
     
   -- add this palantiri
   travelnet.targets[ owner_name ][ network_name ][ palantiri_name ] = {pos=pos, timestamp=os.time() };

   -- do we have a new node to set up? (and are not just reading from a safefile?)
   if( meta ) then

      minetest.chat_send_player(player_name, "Palantiri '"..palantiri_name.."' has been added to the network '"
                                          ..network_name.."', which now consists of "..( anz+1 ).." palantiri(s).");

      meta:set_string( "palantiri_name",    palantiri_name );
      meta:set_string( "palantiri_network", network_name );
      meta:set_string( "owner",           owner_name );
      meta:set_int( "timestamp",       travelnet.targets[ owner_name ][ network_name ][ palantiri_name ].timestamp);

      meta:set_string("formspec", 
                     "size[12,10]"..
                     "field[0.3,0.6;6,0.7;palantiri_name;Palantiri:;"..   meta:get_string("palantiri_name").."]"..
                     "field[0.3,3.6;6,0.7;palantiri_network;Network:;"..meta:get_string("palantiri_network").."]" );

      -- display a list of all palantiris that can be reached from here
...
travelnet.on_receive_fields = function(pos, formname, fields, player)
   ...
   if( meta:get_string("palantiri_network")=="" ) then

      travelnet.add_target( fields.palantiri_name, fields.palantiri_network, pos, name, meta, fields.owner_name );
      return;
   end
   ...
   if(  not( owner_name  ) 
     or not( palantiri_name ) 
     or not( palantiri_network )
     or not( travelnet.targets[ owner_name ] )
     or not( travelnet.targets[ owner_name ][ palantiri_network ] )) then


      minetest.chat_send_player(name, "Error: There is something wrong with the configuration of this palantiri. "..
   ...
   if( this_node ~= nil and this_node.name == 'travelnet:elevator' ) then 
      for k,v in pairs( travelnet.targets[ owner_name ][ palantiri_network ] ) do
         if( travelnet.targets[ owner_name ][ palantiri_network ][ k ].nr  --..' ('..tostring( travelnet.targets[ owner_name ][ palantiri_network ][ k ].pos.y )..'m)'
               == fields.target) then
            fields.target = k;
         end
      end
   ...
   -- if the target palantiri is gone
   if( not( travelnet.targets[ owner_name ][ palantiri_network ][ fields.target ] )) then

      minetest.chat_send_player(name, "Palantiri '"..( fields.target or "?").." does not exist (anymore?) on this network.");
      travelnet.update_formspec( pos, name );
      return;
   end


   if( not( travelnet.allow_travel( name, owner_name, palantiri_network, palantiri_name, fields.target ))) then
      return;
   end
   minetest.chat_send_player(name, "Initiating transfer to palantiri '"..( fields.target or "?").."'.'");
   ...
   -- close the doors at the sending palantiri
   travelnet.open_close_door( pos, player, 1 );

   -- transport the player to the target location
   local target_pos = travelnet.targets[ owner_name ][ palantiri_network ][ fields.target ].pos;
   ...
   if( node2 ~= nil and node2.name ~= 'ignore' and node2.name ~= 'travelnet:travelnet' and node2.name ~= 'travelnet:elevator') then
      ...
      local oldmetadata = { fields = { owner           = owner_name,
                                       palantiri_name    = fields.target,
                                       palantiri_network = palantiri_network }};
...
travelnet.remove_box = function( pos, oldnode, oldmetadata, digger )
   ...
   if( not( oldmetadata ) or oldmetadata=="nil" or not(oldmetadata.fields)) then
      minetest.chat_send_player( digger:get_player_name(), "Error: Could not find information about the palantiri that is to be removed.");
      return;
   end

   local owner_name      = oldmetadata.fields[ "owner" ];
   local palantiri_name    = oldmetadata.fields[ "palantiri_name" ];
   local palantiri_network = oldmetadata.fields[ "palantiri_network" ];

   -- palantiri is not known? then just remove it
   if(  not( owner_name ) 
     or not( palantiri_name ) 
     or not( palantiri_network ) 
     or not( travelnet.targets[ owner_name ] )
     or not( travelnet.targets[ owner_name ][ palantiri_network ] )) then
      ...
      minetest.chat_send_player( digger:get_player_name(), "Error: Could not find the palantiri that is to be removed.");
      return;
   end

   travelnet.targets[ owner_name ][ palantiri_network ][ palantiri_name ] = nil;
   
   -- inform the owner
   minetest.chat_send_player( owner_name, "Palantiri '"..palantiri_name.."' has been REMOVED from the network '"..palantiri_network.."'.");
   if( digger ~= nil and owner_name ~= digger:get_player_name() ) then
      minetest.chat_send_player( digger:get_player_name(), "Palantiri '"..palantiri_name.."' has been REMOVED from the network '"..palantiri_network.."'.");
   end
